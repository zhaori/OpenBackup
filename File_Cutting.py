...
class split_file(object):
    ...
    @staticmethod
    def _new_thead(task, arg):
        thead_list = [Thread(target=task, args=(arg,))]
        for i in thead_list:
            i.start()
        for i in thead_list:
            i.join()
    ...
    def merge(self):
        ...
        for i in os.listdir(self.folder):
            self._new_thead(self._task_write, os.path.join(self.folder, i))
...
if __name__ == '__main__':
    split_file(r'.\1.zip', r'.\Temp', r'.\2.zip', read_size=1024 * 1024, del_cache=True).excision()
