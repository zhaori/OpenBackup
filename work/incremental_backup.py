...
增量备份：设：y为当天备份总数据 z为当前新的数据，a为完整备份文件，x为当天。用！=表示两者差异部分的数据，表达式为下
y = a+(z!=(x-1))
解释：除了首次备份，所有的备份都是用新数据与上次备份的数据进行比较，然后每次备份的数据只是当期比较的差异数据
例如：我有个文件夹存放的有a b c 三个元素，然后
第一天新增了 d 那么第一次开始增量备份 是 abc: d
第二天新增了e 那么第二次的增量备份是 abc: e
第三天删除了a,那么第三次增量备份是 abc: bc
"""
import os
import shutil
from tkinter.messagebox import showinfo

from Lib.Pydos import sys_copy, del_folder, del_file
...
def original_file():
    ...
    copy_full = os.path.join(fr'.\backups\incremental\{basename_folder}', f"{basename_folder}.7z")
    ...
    copy_full_db = os.path.join(fr'.\backups\incremental\{basename_folder}', f"{basename_folder}.db")
...
add_backup = set()  # 添加入备份的集合，用现存数据与原始备份数据对比得到的
del_backup = set()  # 删除文件/文件夹的集合，是用原始备份数据对比现存数据得到的


def incremental_backup():
    if original_file()[0] not in os.listdir(fr'.\backups\incremental\{os.path.basename(READ_DB)}'):
        # 如果完全备份文件不存在就拷贝过来
        copy_ready()
    ...
    basename_folder = os.path.basename(READ_DB)  # 不包含文件夹路径，只取文件夹名字
    new_backup = r'.\backups\incremental\{}\{}'.format(basename_folder, time)  # 生成以日期命名的文件夹
    ...
    for data in add_backup:
        if os.path.isdir(data):
            shutil.copytree(data, os.path.join(TEMP_PATH, os.path.basename(data)))
    ...
    os.system(r'{} -mx5 -t7z a {} {}\* -mmt -sdel'.format('7z', '{}'.format(new_backup), TEMP_PATH))
